...
from load_replica import ReplicaDatasetCache
...
def create_nerf(args):
    ...
    if args.i_embed==1:
        if args.variation == '3RGB':
            print('use 3RGB models')
            model = HashNeRF3RGB(num_layers=2,
                        hidden_dim=64,
                        geo_feat_dim=15,
                        num_layers_color=3,
                        hidden_dim_color=64,
                        input_ch=input_ch, input_ch_views=input_ch_views).to(device)
        elif args.variation == 'DRGB':
            print('use DRGB models')
            model = HashNeRFDRGB(num_layers=2,
                        hidden_dim=64,
                        geo_feat_dim=15,
                        num_layers_color=3,
                        hidden_dim_color=64,
...
def config_parser():
    ...
    parser.add_argument("--variation", type=str, 
                        help='variation for experiments')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        # print(f'device!!!!{args.bounding_box[0].device}')
        ...
        replica_data_loader = ReplicaDatasetCache(data_dir=args.datadir,
                                                                    train_ids=train_ids, test_ids=[],
                                                                    img_h=240,
                                                                    img_w=320)
        images = replica_data_loader.train_samples["image"]
        poses = replica_data_loader.poses
        # print(f"poses shape {poses.shape}")
        render_poses = replica_data_loader.render_poses
        i_test = replica_data_loader.i_test
        bounding_box = replica_data_loader.bounding_box
        render_poses = render_poses[:60]        
        hwf = [replica_data_loader.img_h, replica_data_loader.img_w, replica_data_loader.focal]
        poses = poses[:,:3,:4]
        args.bounding_box = bounding_box
        # print(images)
        # print(render_poses)
        print('Loaded reeplica', images.shape, render_poses.shape, hwf, args.datadir)
        if not isinstance(i_test, list):
            i_test = [i_test]

        i_test = np.arange(images.shape[0])[::12]
